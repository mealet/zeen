...
pub static CORE_FILES: &[CoreFile] = &[
    ...
    // @ Std Modules Builtin Import
    {
        // project/lib/std
        let std_dir = Path::new("../../lib/std");
...
pub static STD_FILES: &[StdFile] = &[
            "#,
        );

        for entry in fs::read_dir(std_dir).unwrap() {
            let entry = entry.unwrap();
            let path = entry.path();
    ...
    value: {content:?},
        ...
        let out_dir = PathBuf::from(env::var("OUT_DIR").unwrap());
        fs::write(out_dir.join("std_files.rs"), output).unwrap();
    }
    // < Std Modules Builtin Import
}
